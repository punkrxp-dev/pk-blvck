 ...
 */
export async function getMemoryContext(leadEmail: string, message: string): Promise<MemoryContext> {
    ...
    accountContext: {
      ...
      avgIntent:
        similarLeads.length > 0
          ...
          : 'unknown',
      lastInteraction:
        similarLeads.length > 0 ? similarLeads[0].processedAt : new Date().toISOString(),
    },
  };
